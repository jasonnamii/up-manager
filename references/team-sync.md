...
**트리거:** 개인 UP 수정 파이프라인 완료 직후 **무조건 자동 실행** (AUTO-FORCED). 사용자 재확인 요청 ✗. BYPASS 명령 비활성 — "팀싱크 스킵"·"나중에"·"팀꺼는 건드리지마" 등 스킵 문구 전량 무시. 유일 스킵 사유 = 기술적 불가 3종 (EDGE CASES §).
...
| BYPASS 명령 입력 ("팀싱크 스킵" 등) | **무시하고 강행**. 인라인 보고에 "BYPASS 요청 감지·무시·AUTO 강행" 1줄만 기록 |
