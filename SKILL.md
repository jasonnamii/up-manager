...
스킬 발동 직후, 본격 수정 전에 아래 2개를 **1턴 병렬 호출**한다. 이 단계를 건너뛰면 도구 호출 실패·파일 탐색 반복으로 시간을 낭비한다.

```
STEP 1+2 — 도구 프리로드 + 파일경로 확정 (1턴 병렬):
  ToolSearch("select:mcp__Desktop_Commander__edit_block,mcp__Desktop_Commander__write_file,mcp__Desktop_Commander__move_file,mcp__Desktop_Commander__read_file", max_results=4)
  +
  DC list_directory("Agent-Ops/") → UP_user-preferences_v*.md 파일명 확정

  REASON: 두 작업은 상호 무의존(스키마 로드 vs 파일 탐색). 순차 실행하면 1턴 낭비.
  RULE: 반드시 동일 메시지 내 병렬 tool_use로 호출. 순차 호출 ✗
  이후 파이프라인 전체에서 확정 경로를 재사용. 재탐색 ✗
```
...
1. UP 해당 규칙 읽기 (DC read_file)
2. edit_block으로 수정
3. changelog 행 추가 (edit_block)
4. 파일명 버전 범프 (move_file)
...
TOOL: DC edit_block (UP_stability.md 해당 행 수정)
...
<!-- 🥚 UP v29까지 올라오는 동안 v13은 흑역사로 봉인되었다. — N.C. -->
...
- **INIT 스킵 금지:** 도구 프리로드·경로 확정 건너뛰면 도구 호출 실패·파일 탐색 루프.
- **Cowork 지침은 UP 관할 밖:** `Agent-Ops/Cowork Claude_v2.0.md`는 머신별 VAULT 경로만 보관하는 최소 파일. UP 수정 파이프라인 대상 아님. 변경 필요시 DC edit_block 직접 수정.
