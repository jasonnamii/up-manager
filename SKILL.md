...
| 5 | **CHECKLIST_SYNC 동행** — UP 본체 수정 시 HIGH·MID 영향 감지→UP_checklist.md 동기화 필수. LOW는 frontmatter version만 갱신. 스킵 = FAIL | 엔진↔계기판 정합성 유지 |
...
```
FORMAT:
  v{old}→v{new} | QC:①~④✓ | 전파:[대상] | stability:[변동사항] | 체크리스트:[HIGH·MID·LOW+동기화/스킵] | 복붙:[대상 또는 스킵]
